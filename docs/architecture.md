...
- 玩家经验和祝福选择；奖励选择阶段暂停正常战斗和排行榜计时。

当前版本已经打通战斗房的完整局内流程：进入房间、战斗、经验升级、祝福选择、出口选择、切房和最终胜负结算均由 `BattleInstance` 与 `RoomRuntime` 协同完成。

`RoomLayoutCatalog` 提供出生点、怪物、障碍物和陷阱布局，`RoomRuntime` 管理进入、战斗、清空、祝福选择、出口选择和切房阶段，`BattleInstance` 负责在切房时统一销毁并重建静态实体。重建失败时会恢复旧地图、静态实体和玩家位置。障碍物阻挡角色移动，陷阱保持可通行；陷阱系统在角色移动后检测新位置。

尖刺只在目标进入范围时造成一次伤害，离开后重新进入可再次触发。毒池对目标维护单个持续伤害状态，持续停留只刷新有效期，不叠层也不重置下一次伤害的计时。沼泽降低玩家普通移动以及怪物追击、撤退速度，但玩家冲刺不受减速影响。

快照包含实体位置、生命、权威碰撞半径、实体类型、场景对象类型、房间流程、玩家经验、已持有祝福、待选祝福以及短期保留的攻击/死亡事件。
