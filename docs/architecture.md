...
局外成长初始等级均为 1，最高 10：攻击每级 `+8%`、攻速每级 `+4%`、生命每级 `+10%`、移速每级 `+3%`。rcenter 在创建房间前读取所有匹配玩家的成长等级，并连同英雄对应的战斗配置一起传给 battle-server。跨服务协议为兼容旧客户端仍使用字段名 `hero`。
...
- 玩家经验和祝福选择；祝福选择阶段继续推进 World tick，但不计入排行榜战斗时长。
