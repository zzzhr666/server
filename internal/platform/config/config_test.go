...
func TestDefaultUsesLocalAddressesWithoutEnvironmentOverrides(t *testing.T) {
	...
	if cfg.Mongo.URI != "mongodb://localhost:27017" {
		t.Fatalf("MongoAddr.URI = %q, want %q", cfg.Mongo.URI, "mongodb://localhost:27017")
	}
	if cfg.Mongo.Database != "game" {
		t.Fatalf("MongoAddr.Database = %q, want %q", cfg.Mongo.Database, "game")
	}
...
func TestDefaultUsesAddressEnvironmentOverrides(t *testing.T) {
	...
	if cfg.Mongo.URI != "mongodb://mongo:27017" {
		t.Fatalf("MongoAddr.URI = %q, want %q", cfg.Mongo.URI, "mongodb://mongo:27017")
	}
	if cfg.Mongo.Database != "game_test" {
		t.Fatalf("MongoAddr.Database = %q, want %q", cfg.Mongo.Database, "game_test")
	}
