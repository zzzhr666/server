...
type Repository interface {
	...
	StartMatch(ctx context.Context, playerID int64, nickname, hero string, teamSize int) (*rcenter.MatchResult, error)
...
type Service interface {
	// Start 发起单人对局或双人匹配。
	Start(ctx context.Context, playerID int64, nickname, hero string, teamSize int) (*rcenter.MatchResult, error)
	// Cancel 取消等待中的匹配。
...
func (g *GameMatchService) Start(ctx context.Context, playerID int64, nickname, hero string, teamSize int) (*rcenter.MatchResult, error) {
	...
	result, err := g.matchRepo.StartMatch(ctx, playerID, nickname, hero, teamSize)
	if err != nil {
