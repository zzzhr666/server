...
当前实现包含账号、好友、在线状态、局外成长、1-4 人对局匹配、世界聊天、好友私聊、聊天历史分页、实时聊天推送、单人/双人/三人/四人通关时间榜、累计击杀榜、四种初始英雄、房间图战斗、升级祝福、奖励房免费奖励与装备商店、结算奖励，以及断线重连和无人房间清理。
...
| 排行榜 | 1-4 人通关时间分榜记录纯战斗时间，累计击杀榜跨模式统计；结算与金币发放在同一 Redis 事务中幂等完成。 |
| 战斗运行时 | C++20 ECS 使用固定系统顺序，BattleRuntime 负责房间生命周期、固定频率 Tick、快照广播、结算回调和无人房间回收。 |
| 房间与场景 | 房间图和布局定义玩家出生点、怪物、静态障碍物与陷阱；奖励房使用独立 `Rewarding` 阶段和水泉、商店场景标识。 |
| 玩法数值 | 局内基础数值集中在 `battle-server/gameplay/gameplay_config.hpp`；包含升级祝福、一次性免费奖励、怪物灵魂产出和当局装备商店。 |
...
- `match_start.team_size` 可选择 1-4 人对局；单人立即创建房间，2/3/4 人分别进入独立 FIFO 队列。成功后 rcenter 为每个玩家保存活跃对局信息，TCP 完成认证和 `match_resume` 都可恢复该信息。
...
- 连续 10 秒未收到有效 UDP 心跳或输入的会话会标记为断开；所有玩家均断开后，房间等待 90 秒后以 `all_players_disconnected` 原因结束并释放玩家。
...
- 祝福选择仍是战斗房清空后的升级转换阶段；奖励房是独立流程，每名玩家可选择一次回血、攻击、护甲、随机祝福或跳过，然后进入出口选择。
- 近战和远程怪物分别掉落 10 和 5 灵魂。灵魂仅在当局有效，可在奖励房商店购买装备；每名玩家对每件装备限购一次，商品对不同玩家不共享库存。
- `WorldSnapshot` 在所有阶段同步玩家灵魂、攻击伤害、移速、攻击间隔和护甲；奖励房及出口选择阶段仍处理移动、攻击和冲刺输入。
